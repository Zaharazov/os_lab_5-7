...
{
    public:
        Message(MessageTypes messageType, int senderId, int receiverId, int _sizeOfBody, void *_body) : messageType(messageType), senderId(senderId), recieverId(receiverId), sizeOfBody(_sizeOfBody), body(nullptr) 
        {
            if (sizeOfBody > 0) 
            {
    ...
    private:
        bool getMessage(zmq::socket_t& socket, zmq::message_t& typeOfMessage,zmq::message_t& header, zmq::message_t& bodyOfMessage) 
        {
