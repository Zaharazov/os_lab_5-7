...
{
    public:
        ServerNode(int id, int _parentId, int _parentPort, int _registerPort) : AbstractNode(id), parentId(_parentId), parentPort(_parentPort), parentRegPort(_registerPort),registerSocket(context, zmq::socket_type::rep) 
        {
            occupyPort();
    ...
    private:
        ...
        Message execProcessor(Message& message) 
        {
            ...
            for (auto server : this->outerNodes) 
            {
                ...
                message.update(Id, server.first); // узнать в инете
